...
def answer_one():
    ...
    for i, degree in enumerate((1,3,6,9)):
        ...
        pred = linreg.predict(poly.fit_transform(predict))
        output[i,] = pred.reshape(1,100)
...
def answer_two():
    ...
    for degree in range(10):
        ...
        X_poly = poly.fit_transform(X_train.reshape(11,1))
        linreg = LinearRegression().fit(X_poly, y_train)
        X_r2[degree] = linreg.score(X_poly, y_train)
        X_test_poly = poly.fit_transform(X_test.reshape(4,1))
        y_r2[degree] = linreg.score(X_test_poly, y_test)
 

    return (X_r2, y_r2)# Your answer here
...
def answer_four():
    ...
    X_test_poly = poly.fit_transform(X_test.reshape(4,1))
    linreg = LinearRegression().fit(X_poly, y_train)
    linear_r2 = linreg.score(X_test_poly, y_test)
    linlasso = Lasso(alpha=0.01, max_iter = 10000).fit(X_poly, y_train)
    lasso_r2 = linlasso.score(X_test_poly, y_test)
...
def answer_five():
    ...
    features_df = pd.DataFrame({'Features' : features, 'Feature Importances' : feature_importances})
    features_df.sort_values(by = 'Feature Importances', ascending = False, inplace = True)
    
    features = []
    for i in range(5):
        features.append(features_df.iloc[i,1])
...
def answer_six():
    ...
    train_scores, test_scores = validation_curve(SVC(), X_subset, y_subset,
                                                 param_name='gamma',
                                                 param_range=gamma, scoring ='accuracy', cv = 3)
...
# gamma = np.logspace(-4,1,6)
# plt.figure()
# plt.plot(gamma, train_scores,'--',gamma, test_scores,'o-')
# plt.show()
